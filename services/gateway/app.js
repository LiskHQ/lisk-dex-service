...
tempApp.run().then(async () => {
	...
	const gatewayConfig = {
		...
		events: {
			...
			'position.updated': (payload) => sendSocketIoEvent('positionUpdated', payload),
			'position.update.failed': (payload) => sendSocketIoEvent('positionUpdateFailed', payload),
			'remove.liquidity': (payload) => sendSocketIoEvent('removeLiquidity', payload),
