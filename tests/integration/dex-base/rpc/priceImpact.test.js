...
const { priceImpactMetaResponseSchema,goodResponseSchema,priceImpactResponseSchema } = require('../../../schemas/dex-base/priceImpactSchema')
    ...
    describe('is able toReturn the impact price on a swap trade on the market price of the pool.', () => {
        it('returns impact price', async () => {
            try {
				...
				expect(result.data).toMap(priceImpactResponseSchema);
				expect(result.meta).toMap(priceImpactMetaResponseSchema);
            } catch (err) {
